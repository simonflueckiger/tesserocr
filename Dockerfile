...
RUN apt-get update && apt-get install -y \
    ...
    libtiff-dev \
    zlib1g-dev
RUN wget https://github.com/DanBloomberg/leptonica/archive/1.76.0.tar.gz -O /tmp/leptonica.tar.gz && tar -xvf /tmp/leptonica.tar.gz --directory /tmp
ARG CACHE_DATE=2016-01-01
WORKDIR /tmp/1.76.0.tar.gz
RUN ./configure && make &&  make install
WORKDIR /tmp
RUN wget https://github.com/tesseract-ocr/tesseract/archive/4.0.0-beta.2.tar.gz -O /tmp/tesseract.tar.gz && tar -xvf /tmp/tesseract.tar.gz --directory /tmp
WORKDIR /tmp/4.0.0-beta.2.tar.gz
